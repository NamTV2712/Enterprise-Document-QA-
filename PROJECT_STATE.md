...
The TOC-link financial-table fallback is now implemented and verified. Production table discovery preserves the Item-8..Item-9 primary path and follows individual statement TOC links only when the primary path yields no parseable tables. PEP now recovers 29 parseable financial tables through its root Financial Statements anchor, while NVDA/ORCL/AVGO/GS remain covered by the existing fallback. The corpus pipeline appended 31 chunks (PEP 29, HD 2), built immutable generation `nomic-e9b6763-fy2026-pep-table-recovery-20260828-attempt-01` with 9,978 points and corpus fingerprint `sha256:dbfd75d501b5b07d3007ba191127e5f46cc0a9ddb0b3cb44a87867534c7e6fb0`, promoted canonical payloads, and rebuilt the trusted local index green at 9,978 points. Coverage is now 46 clean / 4 degraded / 46 financial-table; deterministic priority retrieval spot-checks pass with average keyword recall 1.0. Phase 1 is rebuilt offline and deterministically against this corpus as artifact `sha256:4098095690678549357475558a5a5c98793e8979b023f5d7617ea3c8759f9c7f`, with `30/30` cases, `61` non-empty queries, byte-identical replay, and `12/12 evidence_ok` decomposed-plan audit. Phase 2 is now official on this binding: `30/30` generations and `30/30` judgments completed with no skipped or parse-invalid records under `selective_packed_v1`; Faithfulness `0.5983`, Answer Relevancy `0.9200`, Context Precision `0.3983`, Overall Judge Average `0.6389`, Citation Correctness `1.0000`, Recall Proxy `1.0000`, and Fallback Accuracy `0.9667`. The run encountered provider 429 retries, so latency is diagnostic only, and generation/judging both use `openai/gpt-oss-120b`; scores are directional rather than cross-model comparable. Full backend suite passes 339 tests.
