...
- Compared with the previous 25-company snapshot (`14 clean / 8 degraded / 3 unusable` = `56% / 32% / 12%`), the 50-company trial improved to `35 clean / 9 degraded / 6 unusable` = `70% / 18% / 12%`. This disproves the earlier pessimistic hypothesis that failure rate would rise at larger sample size; unusable rate stayed stable at `12%`, while degraded rate dropped.
...
- Added 6 extended-corpus `priority=3` evaluation cases for clean 50-company tickers: Visa total assets (`99,627`), Mastercard total assets (`54,157`), Eli Lilly total assets (`112,476`), RTX total net sales trend (`68,920 -> 80,738 -> 88,603`), Coca-Cola competition risk factors, and Visa-vs-Mastercard business risk factors. These keep the official priority `<=2` N=30 benchmark stable while giving a separate post-scale validation slice. Deterministic retrieval verification with `.venv\Scripts\python.exe -m scripts.diagnostics.check_priority_retrieval --priority 3 --top-k 5 --candidate-pool 10` passed for all keyword-bearing cases with average keyword recall `1.0000`.
