...
The TOC-link financial-table fallback is now implemented and verified. Production table discovery preserves the Item-8..Item-9 primary path and follows individual statement TOC links only when the primary path yields no parseable tables. PEP now recovers 29 parseable financial tables through its root Financial Statements anchor, while NVDA/ORCL/AVGO/GS remain covered by the existing fallback. The corpus pipeline appended 31 chunks (PEP 29, HD 2), built immutable generation `nomic-e9b6763-fy2026-pep-table-recovery-20260828-attempt-01` with 9,978 points and corpus fingerprint `sha256:dbfd75d501b5b07d3007ba191127e5f46cc0a9ddb0b3cb44a87867534c7e6fb0`, promoted canonical payloads, and rebuilt the trusted local index green at 9,978 points. Coverage is now 46 clean / 4 degraded / 46 financial-table; deterministic priority retrieval spot-checks pass with average keyword recall 1.0. The previous Phase 1/Phase 2 benchmark remains valid only for the prior `db1e48…` corpus; Phase 1 must be rebuilt offline against this new fingerprint before publishing another evaluation result. Full backend suite passes 339 tests.
