...
The six hard-group priority-3 cases (three fact lookups and three year-over-year multi-hop checks for CVX/JPM/XOM) are now covered by two independent deterministic retrieval replays. All `6/6` cases achieve keyword recall `1.0000`, the expected `financial_table` ticker/section, rank-1 primary evidence, and structured score `10.0000`. These cases do not alter the official priority-2 benchmark. The IBM companion-document audit is also complete and read-only: Item 8 references pages `42–116` of IBM's 2025 Annual Report to Stockholders, and the filing links one relative candidate (`ibm-20251231_d2.htm`) that is absent from the local corpus. The result is `companion_missing`, all validation gates are NO-GO, and no production resolver or data rebuild is authorized. Audit report fingerprint: `sha256:8a4ccc6d930b4f2ce4de1d1aa6bbe269de805e3bca98c6e5d3f2197c11ba27b3` (see `data/diagnostics/ibm_companion_document_audit.json`).
